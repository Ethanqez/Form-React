...
function App() {
  ...
  // ✅ Datos originales
// ✅ Datos originales con más empleados y repeticiones
const dataOriginal = [
  // Camila
  ...
  const [registro, setRegistro] = useState(dataOriginal);
  const fechaI = useRef();
  const fechaF = useRef();
  ...
  const handleChange = (e) => {
    if (!e.target.value) {
      setRegistro(dataOriginal); // reinicia si está vacío
      return;
    }

    const datosFiltrados = dataOriginal.filter(registro => {
  ...
  const handleFilterByDate = () => {
    ...
    // Si alguna fecha está vacía → volver a TODOS
    if (!startValue || !endValue) {
      setRegistro(dataOriginal);
      return;
    }
    ...
    const filtrados = dataOriginal.filter(registro => {
  ...
  return (
    <div id="content">
      ...
      <input type='text' onChange={handleChange} style={{width: "600px", height: "40px"}}/>
      <br/>
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
        <input 
          ...
          style={{ height: "38px", textAlign: 'center', fontSize: "14px", margin: "17px", width: "177px" }} 
